...
public class UpdateUserTests : BaseIntegrationTest
{
    ...
    {
        new UpdateUserCommand("", Faker.Name.FirstName(), Faker.Name.LastName()),
        new UpdateUserCommand(Guid.NewGuid().ToString(), "", Faker.Name.LastName()),
        new UpdateUserCommand(Guid.NewGuid().ToString(), Faker.Name.FirstName(), "")
    };
    ...
    {
        // Arrange
        Result<string> result = await _sender.Send(new RegisterUserCommand(
